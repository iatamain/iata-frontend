...
document.addEventListener('keydown', function(e) {
	if(type_menu == "game"){
		if(e.keyCode == 82) clearBut(); //R
		if(e.keyCode == 67 && e.ctrlKey) copyBut(); //C
		if(e.keyCode == 86 && e.ctrlKey) pasteBut(); //V
		if(e.keyCode == 88 && e.ctrlKey) cutBut(); //X
		if(e.keyCode == 65 && e.ctrlKey) selectAllBut(); //A
	}
...
function trimming(arg){
	...
	for(var i = 0; i < 100; i++){
		...
		for(var j = 0; j < 100; j++){
			if(arg[i][j]){
				if(paste_size_min_x > i || paste_size_min_x == -1) paste_size_min_x = i;
				if(paste_size_max_x < i || paste_size_max_x == -1) paste_size_max_x = i;
				if(paste_size_min_y > j || paste_size_min_y == -1) paste_size_min_y = j;
				if(paste_size_max_y < j || paste_size_max_y == -1) paste_size_max_y = j;
			}
		}
	}
	if(paste_size_min_x == -1){
		paste_size_min_x = 0;
		paste_size_min_y = 0;
		paste_size_max_x = 0;
		paste_size_max_y = 0;
	}
	for(var i = paste_size_min_x; i <= paste_size_max_x; i++){
		for(var j = paste_size_min_y; j <= paste_size_max_y; j++){
			arg[i - paste_size_min_x][j - paste_size_min_y] = arg[i][j];
			arg[i][j] = 0;
		}
	}
}
